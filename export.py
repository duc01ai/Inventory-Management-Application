...
class ExportClass:
    def __init__(self, root, emp_no, emp_name):
       ...
       # Treeview widget
        ...
        txt_Emp_No = ctk.CTkEntry(Add_Cart_Frame, textvariable=self.var_empNo, font=("Times new roman", 18), fg_color="light yellow", state='readonly', width=150, height=28).place(x=100, y=8)

        #Emp Name
        Emp_Name=Label(Add_Cart_Frame, text="Emp Name",font=("goudy old style", 16), bg="#FFFFFF").place(x=370, y=10)
        txt_Emp_Name = ctk.CTkEntry(Add_Cart_Frame, textvariable=self.var_empName, font=("Times new roman", 18), fg_color="light yellow", state='readonly', width=180, height=30).place(x=400, y=8)
...
#================def add|update cart=============
    def add_updatecart(self):
        if self.var_product_id.get() == '':
            ...
            try:
                # Kiểm tra nếu số lượng nhập không phải số dương
                requested_quantity = int(self.var_pQuantity.get())
...
'''
            ...
            for row in sorted_cart_list:
              ...
              # Thêm khoảng trắng vào trước các giá trị Quantity và Discount để chúng nhích về bên trái một chút
                formatted_line = f"{name:<{name_width}}{price:>{price_width+6}.2f}{' ' * (quantity_width - 13)}{quantity:>{quantity_width}}{' ' * (discount_width - 13)}{discount:>{discount_width}}%"
